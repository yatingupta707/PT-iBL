...
def eval_ts_once(dataset, model_class, params, n_agents, seed_offset=0):
    """One forward pass over entire dataset. Returns cumulative R/A/reveal."""
    P     = len(dataset)
...
def eval_ts_multi(dataset, model_class, params, n_agents, n_sims):
    ...
    Returns (r_cum, a_cum, reveal_cum) each shape (N_TRIALS,).
    """
    ...
    return r_sum / n_sims, a_sum / n_sims, v_sum / n_sims
...
def evaluate_model(model_name, model_dir, est_data, comp_data,
    ...
    r_train, a_train, v_train = eval_ts_multi(est_data, model_class, params,
                                              n_agents, n_sims)
    # Competition set (test)
    r_test, a_test, v_test = eval_ts_multi(comp_data, model_class, params,
                                           n_agents, n_sims)
    ...
    summary = {
        ...
        'train': {
            ...
            'reveal_rate': _metrics(v_train, human_reveal_ts_est),
        },
        'test': {
            'r_rate': _metrics(r_test, human_r_ts_comp),
            'a_rate': _metrics(a_test, human_a_ts_comp),
            'reveal_rate': _metrics(v_test, human_reveal_ts_comp),
        },
